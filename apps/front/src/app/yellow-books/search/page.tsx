...
function SearchForm({ initialQuery }: { initialQuery?: string }) {
  return (
    <form action="/yellow-books/search" method="GET" className="flex gap-2">
      <Input
        name="q"
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2">
              <Link
                href={`/yellow-books/search?${new URLSearchParams({
                  ...(category && { category }),
                  ...(query && { q: query }),
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {categories.map((cat) => (
        <Link
          key={cat.id}
          href={`/yellow-books/search?category=${encodeURIComponent(cat.name)}`}
        >
          <Badge
